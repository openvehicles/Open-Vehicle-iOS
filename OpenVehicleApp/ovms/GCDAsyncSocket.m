...
- (BOOL)isIPv6Enabled
{
	...
	if (dispatch_get_current_queue() == socketQueue)
	{
		return ((self->config & kIPv6Disabled) == 0);
	}
	else
	{
		__block BOOL result;
	...
	if (delegateQueue)
	{
		...
		dispatch_async(delegateQueue, ^{
			@autoreleasepool {
                ...
                GCDAsyncSocket *acceptedSocket = [[GCDAsyncSocket alloc] initWithDelegate:self->delegate
...
- (BOOL)preConnectWithInterface:(NSString *)interface error:(NSError **)errPtr
{
	...
	if (interface)
	{
        ...
        connectInterface4 = [interface4 copy];
        connectInterface6 = [interface6 copy];
	}
                ...
                error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{
        @autoreleasepool {
            ...
            NSError *tmpErr = err;
            if (![self preConnectWithInterface:interface error:&tmpErr]){
                err = tmpErr;
                   ...
                   error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{
        @autoreleasepool {
            ...
            if (![self connectWithAddress4:address4 address6:address6 error:&tmpErr])
            {
                err = tmpErr;
                return_from_block;
            }
...
- (void)flushSSLBuffers
{
	...
	dispatch_block_t updateEstimatedBytesAvailable = ^{
		...
		estimatedBytesAvailable = self->socketFDBytesAvailable + [self->sslReadBuffer length];
